...
LIMIT 1 
-- Prague has the hightest sum of invoice totals(273.24), thus it has the best customers

--Q5 Person who has spent the most money (besr customer)
--Solved by joining customer and invoice tables, as both have 'customer_id'
...
JOIN invoice ON customer.customer_id = invoice.customer_id
       ...
       ORDER BY invoice_total desc 
--OUTPUT: R Madhav is the best customer as he spends the most (144.54)
...
WITH best_selling_artist AS(
 ...
 WITH my_cte AS (
              ...
              ROW_NUMBER() OVER(PARTITION BY customer.country ORDER BY COUNT(invoice_line.quantity) DESC) AS RowNo --Use row number to get top 1 purchase of the country
			  FROM invoice_line
